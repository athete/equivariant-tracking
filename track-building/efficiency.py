...
model_paths = [model for model in models if "epoch50" in model]
...
thld = thlds[pt_min]
euclid_net = EuclidNet(n_input=3, n_hidden=40, n_layers=5, n_output=1, c_weight=1e-3,)

euclid_net.load_state_dict(
...
with torch.no_grad():
    ...
    for batch_idx, data in enumerate(test_loader):
        data = data.to(device)
        output = euclid_net(data.x, data.edge_index)
...
np.save(f"track-stats/train_{pt_min}GeV", output)
