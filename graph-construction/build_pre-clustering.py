...
Author: Gage DeZoort
Reproduced from https://github.com/GageDeZoort/interaction_network_paper
"""
...
def cluster_hits(dist_matrix, eps=0.05, k=3):
    clustering = DBSCAN(eps=eps, min_samples=k, metric="precomputed").fit(dist_matrix)
...
def select_segments(
    hits1, hits2, phi_slope_max, z0_max, layer1, layer2, remove_intersecting_edges=False
):
    """
...
def construct_graph(
    ...
    for (layer1, layer2) in layer_pairs:
        ...
        selected, dr, dphi, dz, dR = select_segments(
            hits1,
            hits2,
            phi_slope_max,
            z0_max,
    ...
    pt = hits.pt
    unique_pid_map = {
        pid_old: pid_new for pid_new, pid_old in enumerate(np.unique(pid.values))
    }
            ...
            if barrel_to_LEC or barrel_to_REC:
                ...
                if abs(temp[1]) < abs(z1[l]):
                    if temp[0] > 1:
                        ...
                        print(
                            "     > replacing edge (l1={}, l2={}, z1={:.2f})\n".format(
                                temp[2], temp[3], temp[1]
                            ),
                            "      with      edge (l1={}, l2={}, z1={:.2f})".format(
                                l1[l], l2[l], abs(z1[l])
                            ),
                        )
...
def process_event(
    ...
    hits = select_hits(hits, truth, particles, pt_min=pt_min, endcaps=endcaps).assign(
        evtid=evtid
    )
...
def main():
    ...
    file_prefixes = [
        ...
        if (
            (int(prefix.split("00000")[1]) >= args.start_evtid)
            and (int(prefix.split("00000")[1]) <= args.end_evtid)
        )
    ]
