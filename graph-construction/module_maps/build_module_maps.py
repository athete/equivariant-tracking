"""
Script to prepare a module map for the data-driven graph construction algorithm.
...
hits = pd.concat(
    [
        hits_by_loc.get_group(pixel_layers[i]).assign(layer=i)
...
module_maps = {
    (i, j): np.zeros((np.max(module_labels[i]) + 1, np.max(module_labels[j]) + 1))
...
for i, evtid in enumerate(evtids):
    ...
    hits = (
        hits.loc[hits.groupby(["particle_id", "layer"], as_index=False).r.idxmin()]
    ).assign(evtid=evtid)
    ...
    for lp in layer_pairs:
        ...
        hit_pairs = (
            hits0[keys]
            .reset_index()
            .merge(hits1[keys].reset_index(), on="evtid", suffixes=("_1", "_2"))
        )
