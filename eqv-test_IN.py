...
model.load_state_dict(torch.load("./trained_models/IN_geometric_epoch100_1.5GeV.pt"))
...
with torch.no_grad():
    for theta in torch.linspace(0, 2*torch.pi, 100):
        ...
        for data in test_loader:
            data = data.to(device)
            out = model(torch.matmul(data.x, Ry(theta).to(device)), data.edge_index, data.edge_attr)
...
np.save(f"IN-equv-test-y.npy", metrics)